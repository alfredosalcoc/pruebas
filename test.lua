--[[
	...
	by Sirius (forked & themed for Vicoré)
...
local InterfaceBuild = ''
local Release = "Vicoré UI"
local RayfieldFolder = "Vicoré"
local ConfigurationFolder = RayfieldFolder.."/Configurations"
local ConfigurationExtension = ".vicore"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		vicoreOpen = {Type = 'bind', Value = 'K', Name = 'Vicoré Keybind'},
		-- buildwarnings
		-- rayfieldprompts

	},
	System = {
		usageAnalytics = {Type = 'toggle', Value = false, Name = 'Usage Analytics (removed)'},
	}
}
...
local overriddenSettings: { [string]: any } = {} -- For example, overriddenSettings["System.vicoreOpen"] = "J"
...
local VicoreLibrary = {
	...
	Theme = {
		Default = {
			TextColor = Color3.fromRGB(240, 232, 220),

			Background = Color3.fromRGB(33, 24, 18),
			Topbar = Color3.fromRGB(46, 34, 26),
			Shadow = Color3.fromRGB(16, 12, 10),

			NotificationBackground = Color3.fromRGB(24, 18, 14),
			NotificationActionsBackground = Color3.fromRGB(230, 220, 210),

			TabBackground = Color3.fromRGB(94, 72, 56),
			TabStroke = Color3.fromRGB(110, 85, 65),
			TabBackgroundSelected = Color3.fromRGB(186, 146, 112),
			TabTextColor = Color3.fromRGB(240, 232, 220),
			SelectedTabTextColor = Color3.fromRGB(240, 232, 220),

			ElementBackground = Color3.fromRGB(44, 34, 27),
			ElementBackgroundHover = Color3.fromRGB(52, 40, 32),
			SecondaryElementBackground = Color3.fromRGB(36, 28, 22),
			ElementStroke = Color3.fromRGB(72, 56, 44),
			SecondaryElementStroke = Color3.fromRGB(58, 46, 36),

			SliderBackground = Color3.fromRGB(156, 118, 84),
			SliderProgress = Color3.fromRGB(156, 118, 84),
			SliderStroke = Color3.fromRGB(176, 138, 104),

			ToggleBackground = Color3.fromRGB(36, 28, 22),
			ToggleEnabled = Color3.fromRGB(183, 132, 80),
			ToggleDisabled = Color3.fromRGB(110, 98, 90),
			ToggleEnabledStroke = Color3.fromRGB(206, 156, 110),
			ToggleDisabledStroke = Color3.fromRGB(125, 112, 100),
			ToggleEnabledOuterStroke = Color3.fromRGB(100, 80, 65),
			ToggleDisabledOuterStroke = Color3.fromRGB(70, 60, 54),

			DropdownSelected = Color3.fromRGB(52, 40, 32),
			DropdownUnselected = Color3.fromRGB(36, 28, 22),

			InputBackground = Color3.fromRGB(36, 28, 22),
			InputStroke = Color3.fromRGB(88, 70, 56),
			PlaceholderColor = Color3.fromRGB(200, 186, 170)
		},
...
local Rayfield = useStudio and script.Parent:FindFirstChild('Vicoré') or game:GetObjects("rbxassetid://10804731440")[1]
Rayfield.Name = "Vicore"
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local rayfieldDestroyed = false -- True when VicoreLibrary:Destroy() is called
...
repeat
	...
	toDestroy, Rayfield = Rayfield, useStudio and script.Parent:FindFirstChild('Vicoré') or game:GetObjects("rbxassetid://10804731440")[1]
...
local SelectedTheme = VicoreLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = VicoreLibrary.Theme[Theme]
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("Vicoré | Cannot use Lucide icons as icons library is not loaded (Vicoré)")
	else
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			...
			VicoreLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {getSetting("General", "vicoreOpen")}.`, Duration = 7, Image = 4400697855})
		end
	end
...
function VicoreLibrary:CreateWindow(Settings)
	...
	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				VicoreLibrary:Notify({Title = 'Build Mismatch', Content = 'Rayfield may encounter issues as you are running an incompatible interface version ('.. ((Rayfield:FindFirstChild('Build') and Rayfield.Build.Value) or 'No Build') ..').\n\nThis version of Rayfield is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[getSetting("General", "vicoreOpen")]) and not processed then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				VicoreLibrary:Notify({Title = "Rayfield Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			VicoreLibrary:Notify({Title = "Rayfield Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('Rayfield Configurations Error | '..tostring(result))
			VicoreLibrary:Notify({Title = "Rayfield Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	--	Theme = 'Default',
	--	Icon = 0,
	--	LoadingSubtitle = "by Sirius (forked & themed for Vicoré)",
	--	ConfigurationSaving = {
	--		Enabled = true,
	...
	----VicoreLibrary:Notify({Title = "Rayfield Interface", Content = "Welcome to Rayfield. These - are the brand new notification design for Rayfield, with custom sizing and Rayfield calculated wait times.", Image = 4483362458})
