--[[
	...
	Vicoré Interface Suite

	adapted from Rayfield by Sirius
...
local VicoreFolder = "Vicore"

local ConfigurationFolder = VicoreFolder.."/Configurations"
...
local settingsTable = {
	...
	rayfieldOpen = {Type = 'bind', Value = 'K', Name = 'Vicoré Keybind'},
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		...
		task.spawn(function()
			...
			if isfolder and isfolder(VicoreFolder) then

				if isfile and isfile(VicoreFolder..'/settings'..ConfigurationExtension) then

					file = readfile(VicoreFolder..'/settings'..ConfigurationExtension)
	...
	if not success then 
		...
		if writefile then
			...
			warn('Vicoré no pudo acceder al guardado de configuraciones.')
...
local VicoreInterface = useStudio and script.Parent:FindFirstChild('Vicore') or game:GetObjects("rbxassetid://90972839239820")[1]
...
local vicoreDestroyed = false -- True when VicoreLibrary:Destroy() is called



repeat

	if VicoreInterface:FindFirstChild('Build') and VicoreInterface.Build.Value == InterfaceBuild then
	...
	if not warned then
		...
		warn('Vicoré | Build Mismatch')

		print('Vicoré puede presentar problemas porque estás usando una versión de interfaz incompatible ('.. ((VicoreInterface:FindFirstChild('Build') and VicoreInterface.Build.Value) or 'No Build') ..').\n\nEsta versión de Vicoré está pensada para la build '..InterfaceBuild..'.')
	...
	toDestroy, VicoreInterface = VicoreInterface, useStudio and script.Parent:FindFirstChild('Vicore') or game:GetObjects("rbxassetid://90972839239820")[1] --CAMBIAR ASSET AQUI 
																																       --90972839239820
...
if gethui then
	...
	VicoreInterface.Parent = gethui()

elseif syn and syn.protect_gui then 

	syn.protect_gui(VicoreInterface)

	VicoreInterface.Parent = CoreGui

elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then

	VicoreInterface.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	...
	for _, Interface in ipairs(gethui():GetChildren()) do
		...
		if Interface.Name == VicoreInterface.Name and Interface ~= VicoreInterface then
...
if VicoreInterface.AbsoluteSize.X < minSize.X and VicoreInterface.AbsoluteSize.Y < minSize.Y then
...
local MPrompt = VicoreInterface:FindFirstChild('Prompt')
...
local dragBar = VicoreInterface:FindFirstChild('Drag')
...
VicoreInterface.DisplayOrder = 100
...
local Notifications = VicoreInterface.Notifications



local SelectedTheme = VicoreLibrary.Theme.Vicore
...
local function ChangeTheme(Theme)
	...
	if typeof(Theme) == 'string' then
		...
		resolvedTheme = VicoreLibrary.Theme[Theme]
	...
	if not resolvedTheme then
		...
		if Theme ~= nil then
			...
			warn(`[Vicoré] Unknown theme '{Theme}', falling back to Vicoré palette`)

		end

		resolvedTheme = VicoreLibrary.Theme.Vicore
	...
	VicoreInterface.Main.BackgroundColor3 = SelectedTheme.Background

	VicoreInterface.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar

	VicoreInterface.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar

	VicoreInterface.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow



	VicoreInterface.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor

	VicoreInterface.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor

	VicoreInterface.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor

	if Topbar:FindFirstChild('Settings') then

		VicoreInterface.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor

		VicoreInterface.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	...
	for _, text in ipairs(VicoreInterface:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		...
		warn("Vicoré | Cannot use Lucide icons as icons library is not loaded")

	else

		warn("Vicoré | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('Vicoré no pudo decodificar el archivo de configuración; elimina el archivo y vuelve a abrir Vicoré.') return end



	-- Iterate through current UI elements' flags

	for FlagName, Flag in pairs(VicoreLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("Vicoré | Unable to find '"..FlagName.. "' in the save file.")

			print("The error above may not be an issue if new elements have been added or not been set values.")

			--VicoreLibrary:Notify({Title = "Rayfield Flags", Content = "Rayfield was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
...
local function SaveConfiguration()
	...
	for i, v in pairs(VicoreLibrary.Flags) do
...
function VicoreLibrary:Notify(data) -- action e.g open messages
	...
	task.spawn(function()
		...
		if data.Actions then
			...
			warn('Vicoré | Not seeing your actions in notifications?')
...
local function saveSettings() -- Save settings to config file
	...
	if success then
		...
		if writefile then
			...
			writefile(VicoreFolder..'/settings'..ConfigurationExtension, encoded)
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('Vicoré Settings', 0, true)



	if TabList['Vicoré Settings'] then

		TabList['Vicoré Settings'].LayoutOrder = 1000

	end



	if Elements['Vicoré Settings'] then

		Elements['Vicoré Settings'].LayoutOrder = 1000
...
function VicoreLibrary:CreateWindow(Settings)
	...
	if VicoreInterface:FindFirstChild('Loading') then
		...
		if getgenv and not getgenv().VicoreCached then
			...
			VicoreInterface.Loading.Visible = true



			task.wait(1.4)

			VicoreInterface.Loading.Visible = false
	...
	if not correctBuild and not Settings.DisableBuildWarnings then
		...
		task.delay(3, 
			...
			function() 
				...
				VicoreLibrary:Notify({Title = 'Build Mismatch', Content = 'Vicoré puede presentar problemas porque estás usando una versión de interfaz incompatible ('.. ((VicoreInterface:FindFirstChild('Build') and VicoreInterface.Build.Value) or 'No Build') ..').\n\nEsta versión de Vicoré está pensada para la build '..InterfaceBuild..'.\n\nReingresa y ejecuta el script dos veces.', Image = 4335487866, Duration = 15})		
	...
	if isfolder and not isfolder(VicoreFolder) then

		makefolder(VicoreFolder)
	...
	if Settings.LoadingTitle ~= "Vicoré Interface Suite" then

		LoadingFrame.Version.Text = "Vicoré UI"
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio then
		...
		if isfolder and not isfolder(VicoreFolder.."/Discord Invites") then

			makefolder(VicoreFolder.."/Discord Invites")

		end



		if isfile and not isfile(VicoreFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				...
				writefile(VicoreFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Vicoré RememberJoins is true for this invite, this invite will not ask you to join again")
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(VicoreFolder.."/Key System") then

			makefolder(VicoreFolder.."/Key System")
		...
		if Settings.KeySettings.GrabKeyFromSite then
			...
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					...
					print("Vicoré | "..Key.." Error " ..tostring(Response))

					warn('Consulta docs.sirius.menu para ayuda con integraciones de Vicoré.')
		...
		if isfile and isfile(VicoreFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then

			for _, MKey in ipairs(Settings.KeySettings.Key) do

				if string.find(readfile(VicoreFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						...
						if writefile then
							...
							writefile(VicoreFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)

						end

						VicoreLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				-- Prevents animation from trying to play if the button's callback called VicoreLibrary:Destroy()

				if vicoreDestroyed then
				...
				if not Success then
					...
					print("Vicoré | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= VicoreLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				...
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					...
					VicoreLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
			...
			VicoreInterface.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("Vicoré | "..InputSettings.Name.." Callback Error " ..tostring(Response))
			...
			if Settings.ConfigurationSaving then
				...
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					...
					VicoreLibrary.Flags[InputSettings.Flag] = InputSettings
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("Vicoré | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
			...
			if Settings.ConfigurationSaving then
				...
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					...
					VicoreLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Vicoré | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
			...
			if Settings.ConfigurationSaving then
				...
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					...
					VicoreLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= VicoreLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Vicoré | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
			...
			if not ToggleSettings.Ext then
				...
				if Settings.ConfigurationSaving then
					...
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						...
						VicoreLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Vicoré | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
			...
			if Settings.ConfigurationSaving then
				...
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					...
					VicoreLibrary.Flags[SliderSettings.Flag] = SliderSettings
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			...
			VicoreLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})

		else

			VicoreLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
	...
	if not success then warn('Vicoré tuvo un problema al crear la configuración.') end
...
function VicoreLibrary:SetVisibility(visibility: boolean)
...
function VicoreLibrary:IsVisible(): boolean
...
function VicoreLibrary:Destroy()

	vicoreDestroyed = true

	hideHotkeyConnection:Disconnect()

	VicoreInterface:Destroy()
...
if Topbar:FindFirstChild('Settings') then
	...
	Topbar.Settings.MouseButton1Click:Connect(function()
		...
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['Vicoré Settings'])
...
function VicoreLibrary:LoadConfiguration()
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				VicoreLibrary:Notify({Title = "Configuraciones de Vicoré", Content = "No pudimos habilitar el guardado porque tu exploit no ofrece acceso al sistema de archivos.", Image = 4384402990})
		...
		if success and loaded and not notified then
			...
			VicoreLibrary:Notify({Title = "Configuraciones de Vicoré", Content = "Se cargó la configuración guardada de una sesión anterior.", Image = 4384403532})

		elseif not success and not notified then

			warn('Configuraciones de Vicoré Error | '..tostring(result))

			VicoreLibrary:Notify({Title = "Configuraciones de Vicoré", Content = "Ocurrió un problema cargando tu configuración.\n\nRevisa la consola del desarrollador para más detalles.", Image = 4384402990})
...
if useStudio then
	...
	--local Window = VicoreLibrary:CreateWindow({

	--	Name = "Vicoré Example Window",

	--	LoadingTitle = "Vicoré Interface Suite",
	...
	--		FileName = "Key", -- It is recommended to use something unique as other scripts using VicoreInterface may overwrite your key file

	--		SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script

	--		GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like VicoreInterface to get the key from
	...
	----VicoreLibrary:Notify({Title = "Interfaz Vicoré", Content = "Bienvenido a Vicoré. Estas son las notificaciones con diseño cálido y tiempos calculados por Vicoré.", Image = 4483362458})
	...
	--for themename, theme in pairs(VicoreLibrary.Theme) do
...
task.delay(4, function()
	...
	VicoreLibrary.LoadConfiguration()
